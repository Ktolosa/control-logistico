...
THEMES = {
    ...
    # ... puedes agregar más temas si gustas, la lógica es la misma
}
...
def get_connection():
    try:
        ...
        try:
            if err.errno == 2026:
                config.pop("ssl_ca"); config["ssl_verify_identity"] = False
                return mysql.connector.connect(**config)
        except: pass
        return None
    except: return None
...
def enviar_email_con_adjuntos(destinatario, asunto, cuerpo, archivos_adjuntos):
    ...
    try:
        ...
        msg = MIMEMultipart(); msg['From'] = sender_email; msg['To'] = destinatario; msg['Subject'] = asunto
        msg.attach(MIMEText(cuerpo, 'plain'))
        for nombre, datos in archivos_adjuntos:
            part = MIMEApplication(datos, Name=nombre); part['Content-Disposition'] = f'attachment; filename="{nombre}"'; msg.attach(part)
        server = smtplib.SMTP(smtp_server, smtp_port); server.starttls(); server.login(sender_email, sender_password)
        server.sendmail(sender_email, destinatario, msg.as_string()); server.quit()
        return True, "Enviado"
...
def init_tracking_db():
    ...
    if conn:
        try:
            cur = conn.cursor()
            cur.execute("""CREATE TABLE IF NOT EXISTS tracking_db (id INT AUTO_INCREMENT PRIMARY KEY, invoice VARCHAR(100) NOT NULL, tracking VARCHAR(100) NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, INDEX idx_tracking (tracking), INDEX idx_invoice (invoice));""")
...
def guardar_base_tracking(invoice, lista_trackings):
    ...
    try:
        cur = conn.cursor(); vals = [(invoice, t) for t in lista_trackings]
        cur.executemany("INSERT INTO tracking_db (invoice, tracking) VALUES (%s, %s)", vals)
        conn.commit(); conn.close(); return True, f"{len(vals)} guardados"
    except Exception as e: return False, str(e)
...
def buscar_trackings_masivo(lista_trackings):
    ...
    try:
        import pandas as pd; format_strings = ','.join(['%s'] * len(lista_trackings)); query = f"SELECT tracking, invoice FROM tracking_db WHERE tracking IN ({format_strings})"
        cur = conn.cursor(dictionary=True); cur.execute(query, tuple(lista_trackings)); data = cur.fetchall(); conn.close(); return pd.DataFrame(data)
...
def obtener_resumen_bases():
    ...
    if not conn: return pd.DataFrame()
    try:
        import pandas as pd; query = "SELECT invoice, COUNT(*) as cantidad, MAX(created_at) as fecha_creacion FROM tracking_db GROUP BY invoice ORDER BY fecha_creacion DESC"
        cur = conn.cursor(dictionary=True); cur.execute(query); data = cur.fetchall(); conn.close(); return pd.DataFrame(data)
    except: return pd.DataFrame()
...
def eliminar_base_invoice(invoice):
    ...
    try:
        cur = conn.cursor(); cur.execute("DELETE FROM tracking_db WHERE invoice = %s", (invoice,)); conn.commit(); conn.close(); return True
    except: return False

# --- CSS SUPER ESTÉTICO & ANIMACIONES ---
def load_css(theme_code="light"):
    ...
    <style>
        ...
        /* Aplicar animación de entrada a toda la app */
        .stApp {{ 
            background-color: var(--bg); 
            font-family: 'Segoe UI', sans-serif; 
            color: var(--txt);
            animation: fadeInUp 0.6s cubic-bezier(0.22, 1, 0.36, 1); /* Entrada suave */
            margin-top: -60px; /* Ajuste header oculto */
        }}
        ...
        .menu-btn {{ 
            ...
            transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); /* Efecto elástico */
        ...
        div.stButton > button[kind="primary"] {{ 
            background: linear-gradient(135deg, {t['btn_pri']} 0%, {t['btn_pri']}dd 100%) !important; 
            color: {t['btn_txt']} !important; 
            border: none !important;
        }}
        div.stButton > button[kind="primary"]:hover {{ filter: brightness(110%); }}
        ...
        .kpi-card, [data-testid="stExpander"], div.stForm {{ 
            background: var(--card); 
            border: 1px solid rgba(0,0,0,0.05); border-radius: 16px; 
            padding: 20px; 
        ...
        /* KPIs */
        .kpi-val {{ font-size: 1.8rem; font-weight: 800; background: -webkit-linear-gradient(45deg, var(--txt), var(--pri)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }}
