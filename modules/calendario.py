...
def modal_registro(datos, user):
    ...
    if col_cam.toggle("📷 Cámara"):
        ...
        if img:
            ...
            if c and c[0] not in st.session_state.get('scan_buf', []):
                ...
                st.success(f"Leído: {c[0]}")

    if st.session_state.get('scan_buf'):
        if st.button("Limpiar Escaneos"): st.session_state['scan_buf'] = []; st.rerun()
    ...
    with st.form("frm"):
        ...
        with c1:
            ...
            pin = st.selectbox("Prov", PROVEEDORES, index=PROVEEDORES.index(d_prov), disabled=disabled)
            clin = st.selectbox("Cli", PLATAFORMAS, index=PLATAFORMAS.index(d_plat), disabled=disabled)
        with c2:
            sin = st.selectbox("Serv", SERVICIOS, index=SERVICIOS.index(d_serv) if d_serv in SERVICIOS else 0, disabled=disabled)
            esperados = st.number_input("Esperadas", min_value=1, value=d_esp, disabled=disabled)
            pain = st.number_input("Total Paq", 0, value=int(d_paq), disabled=disabled)
        ...
        com = st.text_area("Nota", d_com, disabled=disabled)
        if not disabled:
